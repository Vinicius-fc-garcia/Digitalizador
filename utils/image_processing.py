...
def find_document_contour(image):
    """
    Combina: Detecção por Área Branca + Morfologia para incluir manuscrito + Canny para cantos precisos.
    (Baseado na sua lógica de detecção robusta).
    """
    ...
    # 2. DETECÇÃO DE ÁREA BRANCA (Papel)
    # A binarização de Otsu e o thresholding são usados para isolar a área do papel.
    _, thresh = cv2.threshold(blurred, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    mask_white = cv2.inRange(thresh, 180, 255)
    
    # 3. Morfologia para UNIR documento impresso + manuscrito/sujeira na borda
    # Isso preenche os espaços entre o texto e a caligrafia.
    ...
    # 4. Canny nas bordas externas
    edges = cv2.Canny(blurred, 50, 150, apertureSize=3)
    
    # 5. Combina máscara branca + bordas Canny
    ...
    # 8. Ordena por área (e mantém apenas o maior que é razoável)
    contours = sorted(contours, key=cv2.contourArea, reverse=True)
    img_area = h * w
    
    # 9. Procura o melhor contorno de 4 lados para a correção de perspectiva
    for cnt in contours[:5]:
        ...
        # Itera em epsilons para encontrar o melhor polígono de 4 lados
        for epsilon in [0.015, 0.02, 0.025, 0.03, 0.035, 0.04]:
            approx = cv2.approxPolyDP(cnt, epsilon * peri, True)
            
            if len(approx) == 4:
                # Verifica a proporção para garantir que é um retângulo grande
    ...
    # FALLBACK: Usar o retângulo mínimo (MinAreaRect) no maior contorno
    # Se approxPolyDP falhar, esta é a segunda melhor opção para perspectiva.
    print("⚠️ Não encontrou 4 lados perfeitos. Usando minAreaRect como Fallback...")
    if len(contours) > 0:
        cnt = contours[0]
        rect = cv2.minAreaRect(cnt)
        box = cv2.boxPoints(rect)
        print(f"  MinAreaRect: Ângulo={rect[2]:.1f}°, {rect[1][0]:.0f}x{rect[1][1]:.0f}px")
...
def process_image(image):
    """
    Aplica detecção robusta de contorno e correção de perspectiva.
    """
    orig = image.copy()
    h, w = image.shape[:2]
    ...
    # Margem mínima de 5px para não cortar as bordas
    margin = 5
    maxWidth += 2 * margin
    maxHeight += 2 * margin
    
    # 5. Define pontos de destino para a transformação (retângulo perfeito)
    dst = np.array([
        [margin, margin],
    ...
    # 6. Aplica transformação de perspectiva
    M = cv2.getPerspectiveTransform(pts, dst)
    warped = cv2.warpPerspective(orig, M, (maxWidth, maxHeight))
    
    # 7. Pós-processamento de qualidade
    warped = enhance_document(warped)
    
    print(f"✓ Documento digitalizado: {maxWidth}x{maxHeight}px (Correção de Perspectiva Aplicada)")
    return warped
...
# Substitua 'image_4191c6.jpg' pelo nome do seu arquivo de entrada (se for diferente)
# e 'digitalizado_final.jpg' pelo nome do arquivo de saída desejado.

# scan_document('image_4191c6.jpg', 'digitalizado_final.jpg', show_debug=True)
